...
from .forms import ResumeForm ,ResumeAddress, ResumeLanguage, ResumeEducation, ResumeSkills, ResumeHobbies , ResumeHistory, ResumeReferences,ResumeT
...
class Choose(TemplateView):
    template_name = 'builder/choose.html'
    def post(self,*args,**kwargs):
        form = ResumeT(self.request.POST)
        template = self.request.POST.get('template')
        if form.is_valid():
            resume = ResumeTemp()
            resume.resume_template = template
            form.save()
            return redirect(reverse('builder:edit'))
